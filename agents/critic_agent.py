...
class CriticAgent:
    ...
    def review(
        ...
        result_chat = self.llm.chat(self.SYSTEM_PROMPT, user_prompt, fallback=fallback)
        self.last_llm_meta = result_chat
        review_note = result_chat["content"]
    ...
    def review_batch(self, triage_decisions: List[TriageDecision]) -> str:
        ...
        # Build deterministic fallback with actual data
        lines = ["**Critic Review**\n"]
        ...
        user_prompt = (
            f"Review these {len(triage_decisions)} triage decisions:\n\n"
            + "\n".join(
                f"- {d.incident_id}: priority={d.priority_score}, trust={d.trust_score}, "
                f"human_review={d.human_review_required}, flags={[f.code for f in d.risk_flags]}"
                for d in triage_decisions[:10]
            )
            + "\n\nHighlight incidents with low trust but high priority, missing evidence, "
            "hallucination risk, or security gaps. List which ones require human review and why. "
            "Use markdown bullets. Be concise."
        )
